...
![Edited 1](edited_1.png "Edited 1")
...
![Edited 2](edited_2.png "Edited 2")
...
![Edited 3](edited_3.png "Edited 3")
...
# Accessory functions
Two accessory functions to help the sequences managment are present in the package (get_genbank_table and rename_sequences)  
...
## rename_sequences()
This function rename the sequence in the fasta files provided based on a correspondence table and a vector of markers names.  
...
In this example it is also illustrated how to use the base concatipede function to customize the template preparation, sequence names matching and concatenation.  
...
For this package to work, all the MSAs in fasta format should be placed in the same folder (that can be the working directory). Only the fasta files to be concatenate should be present in the directory. If other fasta files are present and you don´t want to include them, see the `concatipede_prepare(exclude = "")` option.
...
Two accessory functions to help the sequences management are present in the package (`get_genbank_table()` and `rename_sequences()`).
...
```{r eval = FALSE}
get_genbank_table(filename = "Macrobiotidae_seqnames.xlsx", excel.sheet = 1, marker_names = c("COI","ITS2","LSU","SSU"))
```
...
>>>>>>> a25d290f2d356a931c018376c2e1e1298a4cffca
